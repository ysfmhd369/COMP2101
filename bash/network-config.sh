...
myHostname=$(hostname)
myExternalIP=$(curl -s icanhazip.com)
myExternalName=$(getentnhosts $myExternalIP | awk '{print $2}')
myLanInterfaceName=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
myLANAddress=$(ip a s $myLANInterfaceName|awk '/inet /{gsub(/\/.*,"");print $2')
#printing output
cat <<EOF
Hostname        : $myHostname
LAN Address     : $myLANAddress
LAN Hostname    : $(getnet hosts $myLANAddress\ awk '{print $2}')
External IP     : $myExternalIP
External Name   : $myExternalName
EOF
