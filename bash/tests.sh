...
test -f /etc/resolv.conf && echo "/etc/resolv.conf exists and it is a regular file" || echo "/etc/resolv.conf does not exist or it not a regular file"
test -d /etc/resolv.conf && echo "/etc/resolv.conf exists and it is not a directory" || echo "/etc/resolv.conf does not exist or it is not a directory"
test -r /etc/resolv.conf && echo "/etc/resolv.conf exists and it is readable" || echo "/etc/resolv.conf does not exist or it is not readable"
test -w /etc/resolv.conf && echo "/etc/resolv.conf exists and it is readable" || echo "/etc/resolv.conf does not exist or it is not readable"
test -h /etc/resolv.conf && echo "/etc/resolv.conf exists and it is executable" || echo "/etc/resolv.conf does not exist or it is not executable"
# Tests if /tmp is a directory
# TASK 4: Add a test to see if the /tmp directory is readable
test -r /tmp && echo "/tmp exixts and it is readbale" || echo " /tmp does not exist or it is not readbale"
# TASK 5: Add a test to see if the /tmp directory is writable
test -w /tmp && echo "/tmp exist and it is writable" || echo "/tmp does not exist or it is not writable"
...
read secondNumber
[ $(( $fistNumber%2)) -eq 0 ] && echo "$fistNumber is evern" || echo "$firstNumber is odd"
[ $(( $secondNumber%2)) -eq 0 ] && echo "$secondNumber is evern" || echo "$secondNumber is odd"
[ $(( $fistNumber%$seconNumber)) -eq 0 ] && echo "$secondNumber is is a multiple of $firstNumber" || echo "$secondNumber is not a multiple of $firstnumber"
